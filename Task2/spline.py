...
if __name__ == "__main__":
    mat = read_matrix("./data/t")
    # print_float_matrix(mat)
    a = Spline(mat, newton.newtonSecondDeriativeThird(mat, 0), newton.newtonSecondDeriativeThird(mat, 7))
    ...
    ax = []
    ay = []
    for i in [7 / 20 * i for i in range(20)]:
        ax.append(i)
        ay.append(a.approximate(i))
    plot = px.line(x=x, y=y)
    plot.add_scatter(x=ax, y=ay)
    plot.show()